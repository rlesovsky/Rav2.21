...
function Skeleton() {
  return (
    <div className="bg-[#0e2140] backdrop-blur-sm border border-[#1c3253] rounded-xl p-6 h-32 animate-pulse">
      ...
      <div className="h-12 bg-[#152846] rounded w-full" />
    </div>
  )
}
  ...
  if (error) {
    return (
      <div className="bg-[#0e2140] border border-[#1c3253] rounded-xl p-6">
        <h2 className="text-[#e8f0fb] font-medium mb-4">State Timeline (24-Hour)</h2>
  ...
  return (
    <div className="bg-[#0e2140] backdrop-blur-sm border border-[#1c3253] rounded-xl p-6 hover:border-[#2f86d8] hover:bg-[#0e2140] transition-all duration-200">
      <div className="flex items-center gap-1 mb-4">
        <h2 className="text-[#e8f0fb] font-medium">State Timeline (24-Hour)</h2>
        <InfoTooltip
      ...
      <div className="bg-[#152846] rounded-lg p-4">
        <div
          className="flex h-8 rounded overflow-hidden"
          style={{ minHeight: '2rem' }}
        >
          {points.length === 0 ? (
            <div className="flex-1 bg-[#152846] rounded" />
          ) : (
            points.filter((_, i) => i % step === 0).map((p, i) => (
              <div
                ...
                style={{
                  width: `${segmentWidth}%`,
                  backgroundColor: STATE_COLORS[p.state] ?? '#5b7193',
                  minWidth: '2px',
                }}
        ...
        <div className="flex justify-between mt-2 text-[#67809f] text-xs font-mono">
          {points.length > 0 && (
            <>
      ...
      <div className="flex gap-4 mt-3">
        {Object.entries(STATE_COLORS).map(([name, color]) => (
          <span key={name} className="flex items-center gap-1.5 text-sm text-[#9fb4d2]">
