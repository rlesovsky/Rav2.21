...
**`POST /i3x/objects/list`** — request:
```json
{ "elementIds": ["Dataset:path/Tag1"] }
```
Response (spec-shape array, NOT dict-keyed like `/value` and `/history`):
```json
[
  {
    "elementId": "Driftwood Historian:Driftwood Dairy/El Monte CA/Raw Side/Seperator/1/Edge/Motor Amps",
    "displayName": "Motor Amps",
    "typeId": "Tag",
    "parentId": "Driftwood Historian:Driftwood Dairy/El Monte CA/Raw Side/Seperator/1/Edge",
    "isComposition": false,
    "hasChildren": false,
...
Timebase is mixed: dict-keyed for time-series reads (`/value`, `/history`), array for browse (`/list`). The consumer client accepts either shape on `/list` defensively, but this is the captured truth on this historian.
...
2. **`/value` and `/history` return a dict keyed by elementId**, not an array. Cursor's instinct will be array-of-objects with an `id` field. Wrong. **But `/objects/list` IS an array** — see §2.4. Don't assume one shape across the API.
